...
const char* awsEndpoint = "<Your_endpoint>-ats.iot.<your_region>.amazonaws.com";
...
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXgwDQYJKoZIhvcNAQEL
...
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXLX7leOL1r4W9zeT6hb
...
sprintf(fakeData,"{\"state\": {\"reported\": {\"uptime\": %lu, \"temperature\": %f, \"humidity\": %f, \"message\": \"%s\" }}}",millis()/1000,Tem, Hum,Mes); 
  ...
  if (millis() - lastPublish > 10000) {
   ...
   pubSubClient.publish("$aws/things/<YOUR-THING-NAME>/shadow/update", fakeData);
...
void pubSubCheckConnect() {
  if ( ! pubSubClient.connected()) {
    ...
    while ( ! pubSubClient.connected()) {
      Serial.print(".");
      pubSubClient.connect("<YOUR-Unique-ClientId>"); //If using Shadow service then topic publisher and subscriber must have a
