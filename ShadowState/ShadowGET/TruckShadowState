 ...
 * MQTT_MAX_PACKET_SIZE 128 --> 512 if using AWS Device Shadow for subscribtions on the device
...
char* timePointer;  //definitly not constant
...
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXD8VdMgwDQYJKoZIhvcNAQEL
...
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXgLX7leOL1r4W9zeT6hb
...
//-----BEGIN CERTIFICATE-----
"XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXhkiG9w0BAQsF" \
...
void setup() {
  ...
  Serial.println("ESP8266 AWS Shadow IoT Example");
...
//choose your own number of variables and data types, but avoid data buffer size overload
  int weight = random(50000,1000000); //fake number range, adjust as you like
  char * Mes = "empty";
...
sprintf(fakeData,"{\"state\": {\"reported\": {\"ConnectTime\": \"%.19s\", \"Truck weight\": %d, \"lat\":%2.7f, \"long\": %3.7f, \"message\": \"%s\"}}}",timePointer,weight,latt,lon,Mes);

  if (millis() - lastPublish > 10000) {
   pubSubClient.publish("$aws/things/truck3/shadow/update", fakeData);
   Serial.print("Published: "); Serial.println(fakeData);
       delay(500);
   pubSubClient.publish("$aws/things/truck1/shadow/get", "{}");
   Serial.print("Published: "); Serial.println("{}");
       delay(500);
   pubSubClient.publish("$aws/things/truck2/shadow/get", "{}");
    Serial.print("Published: "); Serial.println("{}");
    lastPublish = millis();
  } //end if   
} // end loop
...
void pubSubCheckConnect() {
  if ( ! pubSubClient.connected()) {
    ...
    while ( ! pubSubClient.connected()) {
      Serial.print(".");
      pubSubClient.connect("truck3"); //If using Shadow service then topic publisher and subscriber must have DIFFERENT ClientID here.
    }
    Serial.println(" connected");
     pubSubClient.subscribe("$aws/things/truck1/shadow/get/accepted");
     pubSubClient.subscribe("$aws/things/truck2/shadow/get/accepted");
