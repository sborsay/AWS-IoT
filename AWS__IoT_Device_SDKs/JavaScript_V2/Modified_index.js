 ...
 * Modifications by Stephen Borsay
 */
...
async function execute_session(connection, argv) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let op_idx = 0; op_idx < argv.count; ++op_idx) {
                const publish = async () => {
					...
					//add our custom code
					var myPayload = JSON.stringify({      
                         "Device_ID":   "raspberry33",
                         "temperature": Math.floor(Math.random() * 120) + 1  ,
	   ...
	   //end our code
                    ...
                    connection.publish(argv.topic, myPayload, mqtt.QoS.AtLeastOnce);   //MQTT publish occurs here
                }
                setTimeout(publish, op_idx * 1000); //change to 3 seconds
            }
        }
        catch (error) {
            reject(error);
